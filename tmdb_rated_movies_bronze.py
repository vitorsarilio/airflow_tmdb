...
def extract_tmdb_rated_movies(**context):
    # Configurações
    ...
    base_url = f"https://api.themoviedb.org/3/account/{account_id}/rated/movies"
    ...
    try:
        ...
        # 2. Transformação
        df = pd.DataFrame(all_movies)[["id", "original_title", "title", "rating"]]
        ...
        daily_gcs_path = f"tmdb/bronze_local/rated_movies/rated_movies_{processing_date}.csv"
        ...
        # Arquivo diário de rated_movies
        ...
        # 6. Upload para o GCS
        try:
            # Upload do arquivo diário (rated_movies)
            gcs_hook.upload(
            ...
            try:
                ...
                df.to_csv(
                    f"{backup_dir}/rated_movies_{timestamp}.csv",
                    sep=';',
                    index=False,
                    encoding='utf-8-sig'
                )
...
with DAG(
    ...
    extract_rated_movies_bronze_task = PythonOperator(
        task_id='extract_tmdb_rated_movies_bronze',
        python_callable=extract_tmdb_rated_movies,
        provide_context=True
    )

    trigger_create_rated_movies_silver = TriggerDagRunOperator(
        task_id='trigger_create_rated_movies_silver_processing',
        trigger_dag_id="tmdb_rated_movies_silver",
        execution_date='{{ ds }}',
        wait_for_completion=False,
        reset_dag_run=True 
    )

    extract_rated_movies_bronze_task >> trigger_create_rated_movies_silver