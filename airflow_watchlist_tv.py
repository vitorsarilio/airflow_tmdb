...
def extract_tmdb_watchlist_tv_shows(**context):
    # Configurações
    ...
    base_url = f"https://api.themoviedb.org/3/account/{account_id}/watchlist/tv"
    ...
    try:
        ...
        daily_gcs_path = f"tmdb/bronze_local/watchlist_tv_shows/watchlist_tv_shows_{processing_date}.csv"
        ...
        # Arquivo diário de watchlist_tv_shows
        ...
        # 6. Upload para o GCS
        try:
            # Upload do arquivo diário (watchlist_tv_shows)
            gcs_hook.upload(
            ...
            try:
                ...
                df.to_csv(
                    f"{backup_dir}/watchlist_tv_shows_{timestamp}.csv",
                    sep=';',
                    index=False,
                    encoding='utf-8-sig'
                )
...
with DAG(
    ...
    extract_task = PythonOperator(
        task_id='extract_tmdb_watchlist_tv_shows',
        python_callable=extract_tmdb_watchlist_tv_shows,
