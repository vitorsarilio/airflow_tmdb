...
def extract_tmdb_rated_tv_shows(**context):
    # Configurações
    ...
    base_url = f"https://api.themoviedb.org/3/account/{account_id}/rated/tv"
    ...
    try:
        ...
        # 2. Transformação
        df = pd.DataFrame(all_tv_shows)[["id", "original_name", "name", "rating"]]
        ...
        # Caminhos dos arquivos (note o diretório tv_shows_ids para separar dos filmes)
        daily_gcs_path = f"tmdb/bronze_local/rated_tv_shows/rated_tv_shows_{processing_date}.csv"
        ...
        # Arquivo diário de rated_tv_shows
        ...
        # 6. Upload para o GCS
        try:
            # Upload do arquivo diário (rated_tv_shows)
            gcs_hook.upload(
            ...
            try:
                ...
                df.to_csv(
                    f"{backup_dir}/rated_tv_shows_{timestamp}.csv",
                    sep=';',
                    index=False,
                    encoding='utf-8-sig'
                )
...
with DAG(
    ...
    extract_task = PythonOperator(
        task_id='extract_tmdb_rated_tv_shows',
        python_callable=extract_tmdb_rated_tv_shows,
